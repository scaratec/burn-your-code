...
Feature: Geofence Processor OCI Image
  ...
  Scenario: Container starts and becomes healthy
    ...
    Then the container health endpoint "GET /health" should respond
      with status 200 within 10 seconds
  ...
  Scenario: Alert when a horse leaves the pasture
    ...
    When the containerized service receives a Pub/Sub push event
      on "/telemetry-push" for "Lilly":
      """
      {
        "lon": 9.6116704,
        "lat": 53.2275266,
      ...
      """
    Then the Firestore collection "alerts" should eventually
      contain a record for "Lilly":
      | field | value              |
  ...
  Scenario: No alert when a horse remains inside the pasture
    When the containerized service receives a Pub/Sub push event
      ...
      """
    Then the Firestore collection "alerts" should not contain
      a record for "Honey" within 2 seconds
  ...
  Scenario Outline: Container rejects requests with missing mandatory fields
    ...
    When the containerized service receives a malformed push event
      on "/telemetry-push":
      """
      <payload>
      """
