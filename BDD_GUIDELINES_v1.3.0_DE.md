...
### 8.4 Aufdeckung von "Silent Failures" (New in 1.3.0)
...
*   **BDD-Ansatz:** Ein Test darf niemals nur den HTTP-Statuscode des Redirects prüfen (z.B. 302 Found). Er muss zwingend den Zustand des Zielsystems (z.B. Order Service) validieren.
*   **Wahrheitsgehalt:** "Wenn die UI 'Erfolg' sagt, muss die Datenbank 'Bezahlt' sagen." BDD erzwingt diesen Abgleich über Systemgrenzen hinweg.

### 8.5 Modellierung von Parallelität in verteilten Systemen

In verteilten Architekturen (z.B. Checkout vs. Computop) laufen Prozesse oft parallel ab. BDD hilft dabei, diese Parallelität gedanklich und technisch zu strukturieren.

*   **Parallele Pfade:** Identifizieren Sie im Gherkin explizit den "synchronen Pfad" (User-Interaktion) und den "asynchronen Pfad" (Callback/Hook).
*   **Fehlersimulation:** Testen Sie gezielt, was passiert, wenn Pfad A funktioniert, aber Pfad B scheitert. Die Robustheit eines Systems definiert sich dadurch, dass jeder Pfad für sich genommen in der Lage ist, den fachlich korrekten Endzustand herzustellen.
